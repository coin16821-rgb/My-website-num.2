// ====== Основная логика страницы + космический фон (облегчённая) ======
...
function initSpaceBackground() {
  ...
  const NEBULA_TWINKLE_SPEED = 0.22; // 0.1 – медленно, 0.4 – быстрее

  // Последовательность эффектов: 1 -> 2 -> 3 -> 1 ...
  ...
  const ASSETS = [
    ...
    { name: 'galaxy1', type: 'galaxy', size: 120, opacity: 0.85, depth: 0.20, x: 0.12, y: 0.55, spin: 420 },
    { name: 'galaxy2', type: 'galaxy', size: 70,  opacity: 0.52, depth: 0.22, x: 0.88, y: 0.65, spin: 380 },
    { name: 'galaxy3', type: 'galaxy', size: 160, opacity: 0.48, depth: 0.18, x: 0.62, y: 0.16, spin: 460 },
    { name: 'galaxy4', type: 'galaxy', size: 200, opacity: 0.45, depth: 0.21, x: 0.40, y: 0.78, spin: 400 },

    // Планеты (кликабельны)
    { name: 'sun',     type: 'planet', size: 70,  opacity: 0.85, depth: 0.40, x: 0.82, y: 0.28, spin: 320 },
    { name: 'saturn',  type: 'planet', size: 330, opacity: 0.94, depth: 0.44, x: 0.18, y: 0.40, spin: 280 },
    { name: 'jupiter', type: 'planet', size: 280, opacity: 0.94, depth: 0.46, x: 0.74, y: 0.78, spin: 260 },
    { name: 'earth',   type: 'planet', size: 240, opacity: 0.95, depth: 0.52, x: 0.30, y: 0.20, spin: 210 },
    { name: 'venera',  type: 'planet', size: 210, opacity: 0.93, depth: 0.54, x: 0.88, y: 0.52, spin: 250 },
    { name: 'mars',    type: 'planet', size: 40,  opacity: 0.63, depth: 0.96, x: 0.24, y: 0.78, spin: 230 },
    { name: 'neptun',  type: 'planet', size: 230, opacity: 0.93, depth: 0.50, x: 0.08, y: 0.18, spin: 300 },
    { name: 'moon',    type: 'planet', size: 50,  opacity: 0.92, depth: 0.60, x: 0.65, y: 0.88, spin: 260 },
    { name: 'pluton',  type: 'planet', size: 160, opacity: 0.90, depth: 0.62, x: 0.92, y: 0.14, spin: 340 }
  ];
  ...
  // Запустить следующее действие по очереди
  ...
  function createSprite(asset, idx) {
    ...
    // Кликаем только по планетам/галактикам
    if (!isNebula) {
    ...
    for (const s of sprites) {
      ...
      if (s.effect) {
        ...
        if (et >= 1 || et < 0) {
          ...
          const f = Math.sin(Math.PI * et); // 0->1->0
          ...
          if (et < half) {
            ...
            extraScale = 1 + p * 2;    // 1 -> 3 (ограничили до x3)
            overrideOpacity = 1 - p;   // 1 -> 0
          } else {
            const p = (et - half) / (1 - half); // 0..1
            ...
            extraScale = 0.7 + 0.3 * p; // 0.7 -> 1
            overrideOpacity = p;        // 0 -> 1
          }
        }
      }

      // Лёгкий hover-эффект (масштаб)
      const dx = mx - x;
      const dy = my - y;
      ...
      // ZoomReturn управляет прозрачностью для планет/галактик
